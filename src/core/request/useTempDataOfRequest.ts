import { Actor, useSelector, useStore } from "../store";
...
export function useTempDataOfRequest<TRequestActor extends RequestActor<any, any>>(
  ...
  useEffect(() => {
    const sub = resourceExpiresIn$
      .pipe(switchMap((expiresIn) => (expiresIn ? timer(expiresIn) : EMPTY)))
      .subscribe(() => {
        request(undefined);
      });
